...
int main(void)
{
    ...
    auto z_fall_start, z_fall_now, y_pull_start, y_pull_now;
    auto box_start, box_now;
    ...
    while (!(ds3.button(SELECT) && ds3.button(START)))
    {
        ...
        double rotation = (ds3.stick(RIGHT_T) - ds3.stick(LEFT)) * 0.3;
...
//-----------------------------arm-------------------------------------------------//
        ...
        if(right_distance >= 20){
            ...
            {
                sent_z = 0;
                y_top_limit == false ? sent_y = right_x * 1.8 : sent_y = 0;
            }
            recover = false;
            arm_status = 0;
            ...
            if (recover == true)
            {
                ...
                if (arm_status == 2)
                {
                    ...
                    sent_z = -Z_ARM_PWM;
                    auto z_fall_time = std::chrono::duration_cast<std::chrono::milliseconds>(z_fall_start - z_fall_now).count();
                    if (z_bottom_limit == true || z_fall_time > 1500)
                    {
                        sent_z = 0;
                        arm_status = 3;
                ...
                if (arm_status == 3)
                {
                    ...
                    sent_y = -Y_ARM_PWM;
                    double y_pull_time if (y_back_limit == true || y_pull_time > 2500)
                    {
                        sent_y = 0;
                        arm_status = 0;
                    }
